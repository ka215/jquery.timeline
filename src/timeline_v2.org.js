...
;(function( factory ) {
    ...
    $.extend( $.timeline.prototype, {
        ...
        _init: function() {
            ...
            _selector = _self[0].tagName + ( _self[0].id ? '#' + _self[0].id : '' ) + ( _self[0].className ? '.' + _self[0].className.replace(/\s/g, '.') : '' );
            this.selector = _selector.toLowerCase();
            this.isCompleted = false;
            
            this._renderView();
            
            //_self.on( 'initialized.jqtl', _after_init );
            $(window).on( 'initialized.jqtl', $.timeline.prototype.initialized );
            //_self.on( 'loadEvent.jqtl', _load_event );
            _self.on( 'loadEvent.jqtl', this._loadEvent );
console.log( _self );
        ...
        _loadEvent: function() {
            ...
            let _self         = $(this.elem),
                _opts         = this.option,
                _container    = _self.find('.jqtl-container'),
                _ruler_top    = _self.find('.jqtl-ruler-top'),
                _ruler_bottom = _self.find('.jqtl-ruler-bottom'),
